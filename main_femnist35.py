...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, t, p, epoch):
        if p is None:
            ...
            self.count += 1
            out = relu(self.bn1(self.conv1(x, t, None, epoch)))
            self.count = self.count % 2
            self.act['conv_{}'.format(self.count)] = out
            self.count += 1
            out = self.bn2(self.conv2(out, t, None, epoch))
            ...
            self.act['conv_{}'.format(self.count)] = x
            self.count += 1
            out = relu(self.bn1(self.conv1(x, t, p[0], epoch)))
            self.count = self.count % 2
            self.act['conv_{}'.format(self.count)] = out
            self.count += 1
            out = self.bn2(self.conv2(out, t, p[1], epoch))
            out += self.shortcut(x, t, None, epoch)
            out = relu(out)
        return out
...
class ResNet(nn.Module):
    ...
    def forward(self, x, t, p, epoch):
        if p is None:
            ...
            self.act['conv_in'] = x.view(bsz, 3, 32, 32)
            out = relu(
                self.bn1(self.conv1(x.view(bsz, 3, 32, 32), t, p[0], epoch)))
...
def train_projected(args, p, model, device, x, y, optimizer, criterion, feature_mat, task_id, epoch, sim_tasks, sim_scores, every_task_base):
    ...
    flag = True
    if flag: 
        for jj in range(task_id):
            x = torch.cat((x, args.extra_data[jj][0]), 0)
            y = torch.cat((y, args.extra_data[jj][1]), 0)
...
def main(args):
    ...
    from dataloader import femnist35 as data_loader

    data, taskcla, inputsize = data_loader.get(pc_valid=args.pc_valid, seed=args.seed)
    args.extra_data = []
    for t in data.keys():
        indices = np.random.choice(np.arange(data[t]['train']['x'].shape[0]*0.2), int(len(data[t]['train']['x'])))
        args.extra_data.append((data[t]['train']['x'][indices], data[t]['train']['y'][indices]))

    n_task = 35
    acc_matrix = np.zeros((35, 35))
                ...
                for kk, (m, params) in enumerate(model.named_parameters()):
                    if len(params.size()) == 4:
                        for tt in range(task_id):
                            ...
                            if t*1e5 < distribution[cnt][tt]*1e5:
