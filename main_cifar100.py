...
def train_projected(args, p, model, device, x, y, optimizer, criterion, feature_mat, task_id, epoch, sim_tasks, sim_scores, every_task_base):
    ...
    if Flag:
        for j in range(task_id):
            x = torch.cat((x, args.extra_data[j][0]), 0)
            y = torch.cat((y, args.extra_data[j][1]), 0)
...
def main(args):
    ...
    args.extra_data = []
    for t in data.keys():
        indices = np.random.choice(np.arange(data[t]['train']['x'].shape[0]), int(data[t]['train']['x'].shape[0]*0.05))
